...
spark = SparkSession.builder.appName("EnrichGolfEquipmentData").getOrCreate()
...
# HDFS Base Path and file path correction for Golf Equipment
hdfs_base_path = "hdfs://localhost:9000"
category_path = "/user/hadoop/amazon/categories/Golf_Equipment"  # Adjusted for Golf Equipment
file_path = f"{hdfs_base_path}{category_path}"

# Adjusted enriched data save paths for Golf Equipment
enriched_base_path = f"{hdfs_base_path}/user/hadoop/amazon/enriched_categories/Golf_Equipment"  # Adjusted for Golf Equipment
...
# Calculate Value Score = stars / price (handling division by zero), rounded to 2 decimal places
...
# Save paths adjusted as requested for Golf Equipment
...
# Apply the same enrichment logic as for Game Hardware
...
print("Enriched data saved successfully for Golf Equipment.")
